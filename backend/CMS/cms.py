...
class CMSUser(Base, Creatable):
    ...
    email                   = Column(String(0x1000))
    password                = Column(String(0x10000))
...
def index():
    ...
    if request.method == 'GET':
        ...
        user = User.query.filter_by(email=login['email'], password=md5password).first()
        if not user:
            flash('no user', 'error')
            return U.redirect('login')
        # autologin
        login_user(user, login['autologin'])
